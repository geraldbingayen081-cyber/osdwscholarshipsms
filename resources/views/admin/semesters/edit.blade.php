<x-app-layout title="Edit Semester - CSU–Lal-lo">
    <x-slot name="header">
        Edit Semester
    </x-slot>
    ...
    <div class="max-w-2xl mx-auto bg-white rounded-xl shadow-xs border border-slate-200 p-6 sm:p-8">
        <div class="mb-6 border-b border-slate-200 pb-4">
            <h3 class="text-base font-bold text-slate-800">Edit Semester: {{ $semester->name }}</h3>
            <p class="text-xs text-slate-500 mt-1">Under {{ $semester->academicYear->name }}</p>
        </div>

        <form method="POST" action="{{ route('admin.semesters.update', $semester->id) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                >
                    @foreach($semesterNames as $semName)
                        <option value="{{ $semName }}" {{ old('name', $semester->name) === $semName ? 'selected' : '' }}>
                            {{ $semName }}
                        </option>
                    @endforeach
                </select>
